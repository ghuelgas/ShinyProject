    ...
    dashboardBody(
        ...
        tabItems(
            ...
            tabItem(tabName = "bevahiorAndCause",
                    ...
                    br(),
                    box( p("A strong negative linear relation will have a correlation coefficient close to -1, whereas a strong positive one will be close to 1."), background = "navy", width = 6),
            ...
            tabItem(tabName = "stratification",
                    fluidRow(box(selectizeInput(inputId = "selectedState2",
                                                label = "Select State",
                                                choices = sort(unique(JoinDF$State))), width= 4),
                             box(selectizeInput(inputId = "selectedBehavior2",
                                                label = "Select Behavior/Risk factor",
                                                choices = sort(unique(JoinDF$Behavior))), width= 4),
                             box(selectizeInput(inputId = "StratificationCategory",
                                                label = "Select Stratification Category",
                                                choices = sort(unique(JoinDF$StratificationCategory))),  width= 4),
                             box(p("To further focus the scope of the policies or programs to be implemented, here the population is grouped in different Stratification Categories."), width =12,  background = "navy"),
            ...
            tabItem(tabName = "state",
                    fluidRow( box(selectizeInput(inputId = "selectedState",
                                                 label = "Select State",
                                                 choices = sort(unique(JoinDF$State))), width = 12),
